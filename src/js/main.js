...
function transferDataFromATAKToiTAK() {
    const atakHost = document.getElementById('atak-host').value.trim();
...
function transferDataFromiTAKToATAK() {
    const itakUrl = document.getElementById('itak-url').value.trim();
    
    if (itakUrl) {
        // Extract host from iTAK URL
        let host = '';
        try {
            const urlToParse = itakUrl.startsWith('http') ? itakUrl : `https://${itakUrl}`;
            const urlObj = new URL(urlToParse);
            host = urlObj.hostname;
        } catch (e) {
            host = itakUrl.replace(/^https?:\/\//, '').split('/')[0].split(':')[0];
        }
...
function populateiTAKFromATAK() {
    ...
    if (atakHost) {
        ...
        let hostname = atakHost;
        let protocol = 'https';
        
        try {
            if (atakHost.includes('://')) {
                const url = new URL(atakHost);
                hostname = url.hostname;
                protocol = url.protocol.replace(':', '');
            } else if (atakHost.includes('.')) {
    ...
    if (atakUsername) {
        document.getElementById('itak-username').value = atakUsername;
    }
    
    if (atakToken) {
        document.getElementById('itak-token').value = atakToken;
    }
...
function populateATAKFromiTAK() {
    ...
    if (itakUrl) {
        ...
        try {
            ...
            hostname = itakUrl.replace(/^https?:\/\//, '').split('/')[0].split(':')[0];
        }
    ...
    if (itakUsername) {
        document.getElementById('atak-username').value = itakUsername;
    }
    
    if (itakToken) {
        document.getElementById('atak-token').value = itakToken;
    }
...
async function generateQRCode(data, containerId) {
    ...
    try {
        ...
        container.innerHTML = '<div class="qr-placeholder">Error generating QR code</div>';
        return null;
    }
}

function updateATAKQR() {
    const host = document.getElementById('atak-host').value.trim();
    ...
    if (host && username && token) {
        const uri = `tak://com.atakmap.app/enroll?host=${encodeURIComponent(host)}&username=${encodeURIComponent(username)}&token=${encodeURIComponent(token)}`;
...
function updateiTAKQR() {
    const description = document.getElementById('itak-description').value.trim();
    const url = document.getElementById('itak-url').value.trim();
    const port = document.getElementById('itak-port').value.trim();
...
function updateImportQR() {
    ...
    if (url) {
        const uri = `tak://com.atakmap.app/import?url=${encodeURIComponent(url)}`;
...
async function copyURI(type) {
    ...
    switch (type) {
        case 'atak':
            ...
            const username = document.getElementById('atak-username').value.trim();
            const token = document.getElementById('atak-token').value.trim();
            uri = `tak://com.atakmap.app/enroll?host=${encodeURIComponent(host)}&username=${encodeURIComponent(username)}&token=${encodeURIComponent(token)}`;
            break;
        case 'itak':
            const itakDescription = document.getElementById('itak-description').value.trim();
            const itakUrl = document.getElementById('itak-url').value.trim();
            const itakPort = document.getElementById('itak-port').value.trim();
            ...
            try {
                ...
                itakHost = itakUrl.replace(/^https?:\/\//, '').split('/')[0].split(':')[0];
            }
            ...
            uri = `${itakDescription},${itakHost},${itakPort},${itakProtocolFormatted}`;
            break;
        case 'import':
            const importUrl = document.getElementById('import-url').value.trim();
            uri = `tak://com.atakmap.app/import?url=${encodeURIComponent(importUrl)}`;
            break;
    }
...
function getCurrentFormData() {
    ...
    switch (currentTab) {
        case 'atak':
            data.host = document.getElementById('atak-host').value;
            data.username = document.getElementById('atak-username').value;
            data.token = document.getElementById('atak-token').value;
            break;
        case 'itak':
            data.description = document.getElementById('itak-description').value;
            data.url = document.getElementById('itak-url').value;
            data.port = document.getElementById('itak-port').value;
            data.protocol = document.getElementById('itak-protocol').value;
            break;
        case 'import':
            data.url = document.getElementById('import-url').value;
            break;
    }
...
function showProfileSelection() {
    ...
    const selection = prompt(`Select a profile to load:\n\n${profileList}\n\nEnter the profile name:`);
